...
基于 FastAPI 架构兼容 Claude Code / OpenAI 代理服务，当前支持 LM-4.5/4.6/4.7/5 系列模型

## ✨ 核心特性

- 🔌 **兼容性** - 无缝对接Claude Code/OpenAI
- 🧬 **数据库管理** - SQLite / PostgreSQL + Web 后台统一管理 Token
- 🚀 **流式响应** - 高性能 SSE 实时流式输出
...
# 默认使用 SQLite；如需 PostgreSQL，设置 DB_TYPE=postgresql 和 DATABASE_URL
...
项目使用 SQLite 或 PostgreSQL 统一管理 Token，首次运行会自动初始化：

```bash
# 首次运行会自动初始化数据库；默认创建 SQLite 文件 tokens.db
python main.py
...
| 数据库 | SQLite (aiosqlite) / PostgreSQL (psycopg) | - | Token 与请求日志存储 |
